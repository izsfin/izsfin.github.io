...
export async function onRequest(context) {
    ...
    let gitHubPath = ""; if (directMappings[rawPath]) { gitHubPath = directMappings[rawPath];  } else if (paths[rawPath]) { gitHubPath = paths[rawPath]; } else { gitHubPath = rawPath || "site/html/main.html";  }
    const requestedExt = gitHubPath.split('.').pop().toLowerCase();
    const isSensitiveFile = requestedExt === 'lua' || requestedExt === 'luac';    const isOwner = ip === MY_IP;
    const isRoblox = ua.includes("roblox") || request.headers.get('ml-access') === 'true';
    const isSpecialUA = ua.includes("ml/sa");
    if (isSensitiveFile) {  if (!isOwner && !isRoblox && !isSpecialUA && !hasSecret) {  const logData = {  body: { ip, path: url.pathname, domain: host, userAgent, type: 'unauthorized_code_access' },  method: 'POST'  };  waitUntil(loggerHandler(logData, { status: () => ({ json: () => {}, end: () => {} }) }).catch(() => {}));  return new Response("Access Denied: Code files are protected.", { status: 403 });  }  }
    let codeBranch = "main";
