...
async function hashPassword(pass) {
    ...
    const hash = await pbkdf2(pass, salt, 100000, 64, 'sha512');
    return hash.toString('hex');
}
const BLACKLIST = ['nigger','faggot','retard','слив','leaked'];
const hasBlacklist = (str) => BLACKLIST.some(w => str.toLowerCase().includes(w));

async function getGHFile(path) {
    try { const { data } = await octokit.repos.getContent({ owner: OWNER, repo: REPO, path, ref: BRANCH }); return { content: JSON.parse(Buffer.from(data.content, 'base64').toString()), sha: data.sha }; } catch(e) { return null; } }
...
async function handleAuth(req, res) {
    ...
    if (subaction === 'register') {
        ...
        if (!file || file.content.Password !== hashed) return res.status(401).json({ error: 'Bad login' });
    }

    const token = crypto.randomBytes(24).toString('hex');
    await redis.set(`docs:session:${token}`, username, 'EX', 2592000);
    return res.json({ ok: true, username, token });
}

async function handlePosts(req, res) {
    // Пытаемся взять из кэша
    const cached = await redis.get('docs:posts:index');
    if (cached) return res.json({ posts: JSON.parse(cached) });

    // Если в кэше нет — берем из GH
    const indexFile = await getGHFile('docs/index.json');
    const posts = indexFile?.content?.posts || [];
    
    await redis.set('docs:posts:index', JSON.stringify(posts), 'EX', 60);
...
async function handlePost(req, res) {
    ...
    // Кэшируем конкретный пост на час
    const cacheKey = `docs:post:${id}`;
    const cached = await redis.get(cacheKey);
    if (cached) return res.json({ ok: true, post: JSON.parse(cached) });

    const [user, file] = id.split('/');
    const data = await getGHFile(`docs/users/${user}/${file}.json`);
    
    if (!data) return res.status(404).json({ error: 'Not found' });

    const postContent = data.content.Post;
    await redis.set(cacheKey, JSON.stringify(postContent), 'EX', 3600);
    
    return res.json({ ok: true, post: postContent });
}

async function handleCreate(req, res) {
    const { username, token, name, desc, text } = req.body || {};
    
    // Проверка сессии
    const session = await redis.get(`docs:session:${token}`);
    if (session !== username) return res.status(401).json({ error: 'Auth failed' });

    // Валидация контента
    if (!name || name.length < 5 || hasBlacklist(name) || hasBlacklist(text)) {
    ...
    const id = `${username}/${filename}`;

    const newPost = {
        Author: username, PostName: name, PostDesc: desc,
        PostText: text, Date: new Date().toLocaleDateString('ru-RU')
    };

    // 1. Сохраняем сам файл поста
    await createOrUpdateGHFile(`docs/users/${username}/${filename}.json`, { Post: newPost }, `new post: ${name}`);

    // 2. АТОМАРНОЕ ОБНОВЛЕНИЕ ИНДЕКСА
    // Используем механизм блокировки в Redis, чтобы избежать Race Condition
    const lockKey = 'docs:index:lock';
    const locked = await redis.set(lockKey, '1', 'NX', 'EX', 10); // Блокировка на 10 сек
    ...
    if (locked) {
        try {
            ...
            index.unshift({ id, author: username, name, desc, date: newPost.Date });
            
            await createOrUpdateGHFile('docs/index.json', { posts: index.slice(0, 500) }, 'update index');
            await redis.del('docs:posts:index'); // Сбрасываем кэш списка
        } finally {
