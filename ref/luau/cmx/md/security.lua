-- [ luau/cmx/md/security.lua ]

local HttpService = game:GetService("HttpService")
local Security = {}

-- [ КОНФИГ ]
local SECURITY_URL   = "https://izsfin.github.io/ref/luau/cmx/db/security.json"
local VERIFIED_URL   = "https://izsfin.github.io/ref/luau/cmx/db/verified.json"
local DISCORD        = ".gg/TRPZg4Xfkq"
local ROOT_PATH      = "iME/CMX/"
    ...
    for _, name in pairs(PROTECTED_GLOBALS) do
        if getgenv()[name] ~= _originalGlobals[name] then
            ...
            if name == "cx" then
                ThrowError(
                    "092 Fatal Error",
                    "'" .. (moduleName or "Unknown") .. "' attempting to hook getgenv().cx [ Please, change hook function to another in your module ]",
                    true
                )
            else
                ThrowError(
